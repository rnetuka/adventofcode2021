...
namespace Day7 {
    ...
    vector<int> read_input() {
        string line = File::open("input/day07.txt").read();
        return line.split<int>(',');
    }

    int solution_1() {
        vector<int> crabs = read_input();
        int min = min_element(crabs);
        int max = max_element(crabs);
        map<int, int> fuel_consumption;
        for (int depth = min; depth <= max; depth++)
            for (int crab : crabs)
                fuel_consumption[depth] += abs(crab - depth);
        return min_value(fuel_consumption);
    }

    int fuel_requirements(int current, int target) {
    ...
    int solution_2() {
        ...
        for (int depth = min; depth <= max; depth++)
            for (int crab : crabs)
                fuel_consumption[depth] += fuel_requirements(crab, depth);
        return min_value(fuel_consumption);
    }
    ...
    void print_answers() {
        ...
        cout << "Day 7\n";
        cout << "  How much fuel must they spend to align to that position? " << answer_1 << "\n";
        cout << "  How much fuel must they spend to align to that position? " << answer_2 << "\n";

        // Check correct answers
        assert(answer_1 == 344297);
        assert(answer_2 == 97164301);
    }
